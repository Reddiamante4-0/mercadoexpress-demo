    ...
    <div className="w-full max-w-6xl mx-auto p-4 md:p-6 space-y-6 pb-12 text-left">
      ...
      <div className="bg-white rounded-3xl border border-slate-200/60 p-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4 shadow-xs">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-1">
            Analiza los ingresos, ticket promedio por pedido y rendimiento financiero del supermercado.
          </p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <div className="bg-white rounded-2xl border border-slate-200/60 p-5 shadow-xs flex flex-col justify-between">
          <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Ingresos Totales</span>
          <div className="flex items-baseline gap-2 mt-2">
            <span className="text-2xl font-black text-slate-800">{formatPrice(metrics.totalSales)}</span>
          </div>
          <span className="text-[9px] text-green-600 font-bold mt-2">🚀 +15% vs el mes anterior</span>
        </div>

        {/* Metric 2 */}
        <div className="bg-white rounded-2xl border border-slate-200/60 p-5 shadow-xs flex flex-col justify-between">
          <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Ticket Promedio por Pedido</span>
          <div className="flex items-baseline gap-2 mt-2">
            <span className="text-2xl font-black text-slate-800">{formatPrice(avgOrderValue)}</span>
          </div>
          <span className="text-[9px] text-slate-400 font-bold mt-2">Valor medio gastado por carrito</span>
        </div>

        {/* Metric 3 */}
        <div className="bg-white rounded-2xl border border-slate-200/60 p-5 shadow-xs flex flex-col justify-between">
          <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Sección Más Vendida</span>
          <div className="flex items-baseline gap-2 mt-2">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
        ...
        <div className="bg-white rounded-2xl border border-slate-200/60 p-5 shadow-xs lg:col-span-3 space-y-4">
          <div>
            <h3 className="text-xs font-black uppercase tracking-widest text-slate-700">Rendimiento por Categoría</h3>
            <p className="text-[10px] text-slate-400 mt-0.5">Ventas itemizadas por cada departamento del supermercado</p>
          </div>
            ...
            <table className="w-full text-xs text-left">
              <thead>
                <tr className="border-b border-slate-100 text-[10px] uppercase text-slate-400 font-bold tracking-wider">
                  ...
                  <th className="py-2.5 text-right">Contribución</th>
                </tr>
              </thead>
              <tbody>
                {metrics.categorySales.length === 0 ? (
                  <tr>
                    <td colSpan={3} className="py-8 text-center text-slate-400">No hay registros aún.</td>
                  </tr>
                ) : (
        ...
        <div className="bg-white rounded-2xl border border-slate-200/60 p-5 shadow-xs lg:col-span-2 space-y-4 flex flex-col justify-between">
          <div>
            <h3 className="text-xs font-black uppercase tracking-widest text-slate-700">Perspectivas Financieras</h3>
            <p className="text-[10px] text-slate-400 mt-0.5">Análisis generado automáticamente</p>
          </div>
          ...
          <div className="space-y-4 flex-1 mt-4">
            <div className="flex gap-2.5 items-start p-3 bg-slate-50 rounded-xl border border-slate-100">
              ...
              <div>
                <p className="text-xs font-bold text-slate-700">Rendimiento Alto</p>
            ...
            <div className="flex gap-2.5 items-start p-3 bg-slate-50 rounded-xl border border-slate-100">
              ...
              <div>
                <p className="text-xs font-bold text-slate-700">Días Pico de Pedidos</p>
                <p className="text-[11px] text-slate-400 mt-0.5 leading-relaxed">
