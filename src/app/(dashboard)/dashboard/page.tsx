...
export default function DashboardAdminPage() {
  ...
  const getPaymentDetailsShort = (details: string) => {
    if (details.includes('Tarjeta')) return 'Tarjeta';
    if (details.includes('PSE')) return 'PSE';
    return 'Billetera';
  };
    ...
    <div className="w-full max-w-6xl mx-auto p-4 md:p-6 space-y-6 pb-12 text-left">
      ...
      <div className="bg-white rounded-3xl border border-slate-200/60 p-6 flex flex-col md:flex-row md:items-center justify-between gap-4 shadow-xs">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-1">
            Aquí tienes el resumen del inventario y las ventas de MercadoExpress hoy.
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-white rounded-2xl border border-slate-200/60 p-4 shadow-xs flex items-center gap-3">
          ...
          <div>
            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">Ventas Totales</span>
        ...
        <div className="bg-white rounded-2xl border border-slate-200/60 p-4 shadow-xs flex items-center gap-3">
          ...
          <div>
            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">Pedidos</span>
            <span className="text-base font-black text-slate-800 mt-0.5 block">{metrics.totalOrders} órdenes</span>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-200/60 p-4 shadow-xs flex items-center gap-3">
          ...
          <div>
            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">Clientes Únicos</span>
            <span className="text-base font-black text-slate-800 mt-0.5 block">{metrics.activeClients} compradores</span>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-200/60 p-4 shadow-xs flex items-center gap-3">
          ...
          <div>
            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">Alertas Stock</span>
            <span className="text-base font-black text-slate-800 mt-0.5 block">{metrics.lowStockCount} alertas</span>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
        ...
        <div className="bg-white rounded-2xl border border-slate-200/60 p-5 shadow-xs lg:col-span-3 flex flex-col justify-between">
          <div>
            <h3 className="text-xs font-black uppercase tracking-widest text-slate-700">Historial de Ventas</h3>
            <p className="text-[10px] text-slate-400 mt-0.5">Ventas mensuales proyectadas vs reales (ficticias)</p>
          </div>
          ...
          <div className="mt-4 flex justify-between items-center text-[10px] text-slate-400 font-bold">
            ...
            >
              <span>Ver reporte de ventas</span>
        ...
        <div className="bg-white rounded-2xl border border-slate-200/60 p-5 shadow-xs lg:col-span-2 flex flex-col">
          <h3 className="text-xs font-black uppercase tracking-widest text-slate-700">Ventas por Categoría</h3>
          <p className="text-[10px] text-slate-400 mt-0.5">Top de facturación por sección</p>

          <div className="mt-4 flex-1 space-y-3.5 overflow-y-auto max-h-52 pr-1">
            {metrics.categorySales.length === 0 ? (
              <p className="text-xs text-slate-400 py-10 text-center">No hay registros de ventas.</p>
            ) : (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white rounded-2xl border border-slate-200/60 p-5 shadow-xs lg:col-span-2 space-y-4">
          <div className="flex justify-between items-center">
            <div>
              <h3 className="text-xs font-black uppercase tracking-widest text-slate-700">Pedidos Recientes</h3>
              <p className="text-[10px] text-slate-400 mt-0.5">Últimas 5 órdenes de compra</p>
            </div>
            <button 
              onClick={() => router.push('/dashboard/pedidos')}
            ...
            <table className="w-full text-xs text-left">
              ...
              <tbody>
                {recentOrders.length === 0 ? (
                  <tr>
                    <td colSpan={5} className="py-8 text-center text-slate-400">No hay pedidos registrados.</td>
                  </tr>
                ) : (
                  recentOrders.map((order) => (
                    <tr key={order.id} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors">
                      ...
                      <td className="py-3 text-slate-400">{getPaymentDetailsShort(order.paymentDetails)}</td>
                      <td className="py-3">
        ...
        <div className="bg-white rounded-2xl border border-slate-200/60 p-5 shadow-xs space-y-4">
          <div>
            <h3 className="text-xs font-black uppercase tracking-widest text-slate-700">Inventario Crítico</h3>
            <p className="text-[10px] text-slate-400 mt-0.5">Productos con stock menor a 5 unidades</p>
          </div>
          ...
          <div className="space-y-3 overflow-y-auto max-h-56 pr-1">
            {lowStockItems.length === 0 ? (
              <div className="text-center py-10">
                ...
                <p className="text-xs text-slate-400 font-bold">Todo en orden. Stock suficiente en todas las categorías.</p>
              </div>
            ) : (
                ...
                <div key={prod.id} className="flex items-center justify-between p-2.5 bg-slate-50 rounded-xl border border-slate-100">
                  <div className="flex items-center gap-2 min-w-0">
                    ...
                    <div className="min-w-0">
                      <h4 className="font-bold text-slate-800 truncate text-[11px]">{prod.name}</h4>
                      <span className="text-[9px] text-slate-400 block">{prod.category}</span>
                    </div>
                  </div>
                  <span className={`text-[10px] font-black px-2 py-0.5 rounded-md ${
                    prod.stock === 0 ? 'bg-red-100 text-red-700' : 'bg-orange-100 text-orange-700'
                  }`}>
                    {prod.stock === 0 ? 'Agotado' : `${prod.stock} und`}
                  </span>
                </div>
              ))
            )}
